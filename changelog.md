...
(raster) Change one of the bus guideways in the legend to a busway.
...
## Vector 20/06/2026, Raster 28/06/2026
...
Show playground balance beams and playhouses.
...
## Raster 26/04/2026 PM, vector 26/04/2026 AM
...
## Raster 29/03/2026, vector 01/04/2026
...
## Vector 28/03/2026, Raster 29/03/2026
...
Handle `abandoned:railway=rail` as `railway=abandoned`.
Handle `razed:railway=rail` as `railway=dismantled`.
Handle `disused:railway=rail` as `railway=disused`.
...
(raster, vector) Fixed bug https://github.com/SomeoneElseOSM/openstreetmap-carto-AJT/issues/8 where `leisure=horse_riding` were no longer shown as `landuse=unnamedcommercial` if not grass.

## 06/03/2026
(raster, vector) Include some `cycleway:right` values in deciding when there is a `sidewalk`.

## Raster 06/03/2026, vector 20/02/2026
...
(raster, vector) Added `amenity=language_school` to list of nonspecific shops.
...
## Raster 31/12/2025, vector 30/12/2025
...
(raster, vector) Added icon for `amenity=boat_storage`.  Also added to the raster legend.
...
(raster, vector) Handle `designation=adopted_footway` as public footpaths.
...
## Vector 09/10/2025, raster 14/10/2025
...
## Vector, 16/09/2025 raster 21/09/2025
(raster, vector) Removed `office=office`; no longer in OSM data.
(raster, vector) Treat "`natural=water`; `basin=detention`" etc. as intermittent.
...
(raster, vector) Treat Don't set `place=locality` if any otherwise handled `natural` tag is set.
(raster) If a `place=locality` is a `townland` or `subtownland` and an area, remove it, because there will probably be a duplicate point.
...
## Vector 30/08/2025, raster 31/08/2025
...
(raster, vector) Treat area `highway=raceway` as area `leisure=track`.

## 18/07/2025
(vector) Treat linear `natural=strait` and `natural=mountain_range` as a linear `natural=valley`.
(raster) Treat `natural=cliff`, `natural=ridge`, `natural=arch', `natural=strait`, `natural=mountain_range` and `natural=gully` nodes as a geographic place like `cape`, `peninsula` etc. - all treated like `place=locality`.
(raster) Show area `place=locality` at the same zoom levels as `place=island` (varies by size).

## 13/07/2025
(vector) Treat `place=ocean` as `place=sea`.  Other components of vector style will processed based on size.
(raster, vector) Treat `natural=isthmus`, `natural=pass`, `natural=creek`, `natural=fishing_bank`, `natural=inlet`, `natural=land`, `natural=hillside` and `natural=cirque` as a geographic place like `cape`, `peninsula` etc. - all treated like `place=locality`.

## Vector 12/07/2025, raster 13/07/2025
...
(raster) Show `service=spur` at lower zooms (no change needed to vector - that was already OK)

## 03/07/2025
(raster, vector) Show `wheelchair` flash on `amenity=fast_food` with `cuisine=kebab`, `cuisine=mexican` and synonyms.
(raster, vector) Show `wheelchair` flash on `amenity=restaurant` with `cuisine=british`, `cuisine=regional`, `cuisine=mexican`, `cuisine=seafood` and synonyms.
...
## Vector 23/05/2025, Raster 26/05/2025
(raster, vector) Treat `amenity=fast_food; cuisine==frozen_yogurt` as `cuisine=ice_cream`.
(raster, vector) Treat `amenity=fast_food; cuisine==filipino` as `cuisine=chinese`.
(raster, vector) Treat `amenity=fast_food; cuisine==friture` as `cuisine=fish_and_chips`.

## Vector 11/05/2025, Raster 26/05/2025
...
(raster, vector) Shorten some names, where a rare longer version is used in place of a common shorter one.
(raster, vector) Consolidate some National Trail names so that "names" like "King Charles III England Coast Path: East: Shotley Gate to Felixstowe Ferry" are shown instead as just "England Coast Path".
...
(raster, vector) Show more `unclassified` `surface` values as `unpaved`.
(raster, vector) Don't show `bridge=low_water_crossing` on a `waterway` as a `bridge`.
(raster, vector) If a `waterway` is allegedly both a `bridge` and a `tunnel`, assume it is not really a bridge.
...
Add `animal` to the list of keys so that horse walkers with no other keys are not missed.
...
Show [disabled parking spaces](https://taginfo.openstreetmap.org/tags/parking_space=disabled#overview) with distinct icons depending on if free or for pay.
Show [waterway=flowline](https://taginfo.openstreetmap.org/tags/waterway=flowline#overview) as stream.
...
Fixed text placement so that some amenity and leisure names appear in a more sensible position.
